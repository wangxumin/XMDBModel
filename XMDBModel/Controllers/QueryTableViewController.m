    ...
    switch (_type) {
        case 1:
        {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                NSLog(@"第一条");
                XMUserModel *user = [XMUserModel findFirstByCriteria:@" WHERE age = 20 "];
                if (!user)  return;
        ...
        case 2:
        {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                NSLog(@"小于20岁");
                [self.data addObjectsFromArray:[XMUserModel findByCriteria:@" WHERE age < 20 "]];
                [self.tableView reloadData];
            });
            break;
        }
        case 3:
        {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                NSLog(@"全部");
                [self.data addObjectsFromArray:[XMUserModel findAll]];
                [self.tableView reloadData];
            });
            break;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XMUserModel *user = [self.data objectAtIndex:indexPath.row];
