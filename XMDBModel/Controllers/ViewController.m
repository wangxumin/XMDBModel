...
- (IBAction)deleteData:(id)sender {
    [XMUserModel deleteObjectsByCriteria:@" WHERE pk < 10"];
}
...
- (IBAction)deleteData3:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [XMUserModel deleteObjects:array];
    });
}
...
- (IBAction)updateData:(id)sender {
    ...
    dispatch_async(q3, ^{
        ...
        [XMUserModel updateObjects:array];
    });
...
- (IBAction)queryData1:(id)sender {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSLog(@"第一条:%@",[XMUserModel findFirstByCriteria:@" WHERE age = 20 "]);
    });
}
...
- (IBAction)queryData2:(id)sender {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSLog(@"小于20岁:%@",[XMUserModel findByCriteria:@" WHERE age < 20 "]);
    });
}
...
- (IBAction)queryData:(id)sender {
    static int pk = 5;
    NSArray *array = [XMUserModel findByCriteria:[NSString stringWithFormat:@" WHERE pk > %d limit 10",pk]];
    pk = ((XMUserModel *)[array lastObject]).pk;
    NSLog(@"array:%@",array);
}
